...
1. 公式プレイヤーを再生すると、動画下の「今日のレッスン」カードが現在の字幕へ追従します。
2. 一時停止すると字幕と時刻が停止します。公式プレイヤー側での停止・シークにも追従します。
3. 動画上の「前のセリフ」「次のセリフ」で、対象フレーズの `max(0, startMs - 700)` にシークして再生します。プリロール中も移動先の字幕を表示します。
4. 再生バーを動かすとプレビューが変わり、離した時点でシークします。±5秒ボタンも使えます。
5. ヘッダーの「JA / EN」で日本語訳を表示・非表示にできます。
6. 下部の4ボタンから、サンプルAI解説、現在の1文の無限リピート、フレーズ保存、発話練習への切り替えを試せます。
...
決定版モックアップを基準に、Deep Night Slateの画面、フロストピル、超極太字幕、相棒キャラクター、4色の立体ボタンを採用しています。PCでは最大430pxのアプリ枠を中央表示し、スマホでは画面幅に広がります。設計書自体は変更していません。
