...
# Lingoflow Learner Model・データ・計測仕様 v0.6
...
形式別の進級履歴として `exercise_format`、`calendar_date`、`first_attempt_correct`、`extra_hint_used`、`answer_revealed`、`timezone_at_event` を記録する。進級判定は同じ形式内で行い、表現の「習得済み」判定とは別に管理する。
