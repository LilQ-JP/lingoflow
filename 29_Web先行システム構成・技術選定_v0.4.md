...
# Lingoflow Web先行システム構成・技術選定 v0.4
...
- 秘密情報をログへ残さない
- 権限の最小化
- 年齢区分だけを保存し、生年月日を学習プロフィールへ保持しない
- 18歳未満は安全フィルターを解除できない
- 動画、字幕、翻訳、AI解説、問題を個別に報告できる識別子を持たせる
- 低信頼または報告集中時だけ人手確認キューへ送る

## 12.1 テスト期間の解析制限

レート制限は「新規動画解析ジョブ」と「解析済み教材の利用」を別のカウンターにする。

- 新規解析：ユーザーごとに1日3件、月20件
- 解析済み教材：利用回数を制限しない
- システム・素材都合の失敗：新規解析カウンターへ確定計上しない
- 重複動画が共通キャッシュに存在する場合：新規解析として計上しない

API応答は `new_analysis_daily_remaining`、`new_analysis_monthly_remaining`、`library_learning_unlimited` を分離して返し、UIが誤って「学習は月20本まで」と表示できない形にする。
