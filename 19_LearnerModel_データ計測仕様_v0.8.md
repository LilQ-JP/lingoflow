...
# Lingoflow Learner Model・データ・計測仕様 v0.8
...
- 同義表現はAIだけで確定せず、事前登録した許容解またはレビュー可能な判定結果を使う
- 音声回答は認識結果をユーザーが確認した後に採点する
...
| 2回とも失敗 | その表現の当日出題を終了し、`review_required` にする |
| 再出題形式 | 不正解時と同じ形式、または一段易しい形式 |
