...
version: 1.0
status: user-confirmed-rules-added
created: 2026-09-05
updated: 2026-09-06
owner: LilQ
...
# Lingoflow Learner Model・データ・計測仕様 v1.0
...
| `independent` | 異なる2日で補助なしの意味確認に正解し、発話または入力練習を完了 | English |
| `review_required` | 習得後の確認でつまずき、再定着が必要 | HintまたはFull |
| `archived` | ユーザーが学習対象外にした | 出題しない |
...
1日目、ヒントなしで意味を正答     → independent_candidate
別の暦日でもう1回、ヒントなしで正答
かつ発話または入力練習を1回完了   → independent（習得済み）
...
表現を画面上で「習得済み」と数えるには、次の両方を満たす。

1. 異なる2つの暦日に、ヒントを使わず、その表現の文脈上の意味を正しく答える
2. その表現を使う発話練習または文字入力練習を、いずれか1回完了する

同じ日に何度正解しても、意味確認は1日分として数える。発話または入力練習は、2回の意味確認と同じ日でも別の日でもよい。音声を使えないユーザーは文字入力で同じ条件を満たせる。

正答履歴は `calendar_date`、`timezone_at_event`、`hint_used`、`meaning_check_result` を記録し、端末時刻の変更やタイムゾーン移動による重複達成を防ぐ。

### 習得後に間違えた場合

一度「習得済み」になった表現について、その後の復習で不正解、答え表示、またはヒント使用が発生した場合は、現在状態を `review_required` にする。

- 習得実績、初回習得日、習得表現数は保持する
- 完了済みロードマップと章の完了状態は取り消さない
- 次回復習を通常より早め、今日または翌日の復習候補へ優先追加する
- 次の復習で再びヒントなしで意味を正解したら `independent` に戻す。発話・入力練習の再実施は求めない
- 発話認識の失敗だけでは `review_required` にしない

このため、データ上は「過去に習得条件を達成したか」と「現在どれほど補助が必要か」を別々に保持する。
...
|---|---|
| 答え表示／不正解 | 翌日 |
| 習得後の答え表示／不正解 | 今日の残り枠、または翌日 |
| ヒントあり正解 | 2日後 |
| ヒントなし正解 | 4日後 |
| 異なる2日でヒントなし正解し、発話または入力も完了 | 7日後 |
| 7日後もヒントなし正解 | 14日後 |
...
穴埋めは対象表現を文脈の中で思い出す主要な意味確認として使う。習得判定へ使える「ヒントなし正解」は、問題表示前または回答中に訳、先頭文字、文字数、選択肢、答えを表示していない回答に限る。

- 大文字・小文字、全角・半角、句読点など意味を変えない差は許容する
- 活用や語形が文脈に合わない場合は不正解にする
- 正解として許容するのは、対象表現の短縮形、表記揺れ、意味を変えない記号・空白差に限る
- 時制、三単現、動詞活用（`-ed`、`-ing`等）が文脈上誤っている場合は不正解にする
- 文法的に通じる別の言い換えは正解にせず、動画内の対象表現で答えるよう促す
- 音声回答は発話終了後に即時自動採点し、認識誤りには「やり直す」を1タップで提供する
- 答え表示後の再入力は練習完了に使えるが、ヒントなし正解には数えない

### 問題形式の進級・降格

| 判定 | ルール |
|---|---|
| 進級（初挑戦） | 同一形式で、異なる日に2回、追加ヒントなしで初回正解 |
| 降格 | 1回の不正解で即時に一段階下げ、上位形式の進級カウントをリセット |
| 再昇格（最初の降格後） | 降格先で異なる日に1回、追加ヒントなしで初回正解すれば元の形式へ復帰 |
| 再昇格（恩典使用後） | 再度降格した後は、通常どおり異なる日に2回の正解が必要 |
| 答えを見る | 不正解として降格させないが、進級実績にも数えない |
| 同日の反復 | 進級カウントは形式ごとに1日1回まで |

ここでいう「初回正解」は、その出題回で答え表示、再回答、追加ヒントを使う前の最初の確定回答が正しいことを指す。選択肢や単語並べ替えでは、その形式自体に含まれる候補表示は追加ヒントとみなさない。

形式別の進級履歴として `exercise_format`、`calendar_date`、`first_attempt_correct`、`extra_hint_used`、`answer_revealed`、`timezone_at_event` を記録する。さらに表現ごとに `recovery_credit_used`、`demoted_from_format`、`promotion_count_by_format` を保持する。進級判定は同じ形式内で行い、表現の「習得済み」判定とは別に管理する。

初回再昇格恩典は表現ごとに1回だけ使える。初めて降格した時点では `recovery_credit_used = false` のままにし、1回正解で実際に再昇格した時点で `true` にする。

### 当日の再出題

不正解後は答えと該当音声を確認させ、その表現を当日の問題列へランダムに差し戻す。

| 項目 | ルール |
|---|---|
| 再出題位置 | 現在位置から2〜4問後をランダムに選ぶ |
| 進級カウント | 再出題で正解しても算入せず、当日の上限1を維持する |
| 再出題回数 | 一つの表現につき当日最大2回 |
| 2回とも失敗 | その表現の当日出題を終了し、`review_required` にする。翌日以降は一段易しい形式から開始 |
| セッション内の再出題形式 | 不正解時と同じ形式 |

その表現を終了しても学習セッション全体は続行する。記録には `retry_index`、`scheduled_offset`、`retry_format`、`retry_result`、`daily_expression_closed` を追加する。

スキップでは `retry_later_today` と `archive_expression` を区別する。アーカイブした表現は復習キュー、今日の学習、ロードマップの未習得数から除外し、履歴自体は削除しない。ユーザーは後から解除できる。

セッション途中状態はユーザーの学習日付が同じ間だけ復元する。日付が変わった場合は途中位置を破棄し、未完了項目を当日の復習候補へ戻して優先度を再計算する。回答・習得・進級の確定履歴は保持する。

### 学習日の境界

学習日はアカウントに設定されたタイムゾーンの午前4時に切り替える。午前0時から3時59分59秒までの学習は前の学習日に属する。

```text
study_date = local_datetime - 4 hours の暦日
```

この `study_date` を、ストリーク、同日の進級上限、異なる日の正解判定、当日セッション再開、再出題上限に共通利用する。進行中に暦日の午前0時を越えてもセッションは中断しない。午前4時を越えた時点では進行中の問題を安全に終え、その後の未完了項目を新しい学習日のキューへ再編成する。
